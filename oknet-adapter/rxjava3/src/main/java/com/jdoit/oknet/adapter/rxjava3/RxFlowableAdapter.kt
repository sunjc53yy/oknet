 ...
 * @Description:
 */
class RxFlowableAdapter<T>(var scheduler : Scheduler?, var isAsync : Boolean = false)
            : INetWorkerAdapter<T, Flowable<NetResponse<T>>> {
    override fun adapter(worker: INetWorker<T>): Flowable<NetResponse<T>> {
